...
def wsc_unbiased_label(X, y, S, label, delta=0.1, M=1000, test_size=0.75, random_state=2020, verbose=False):
    ...
    if label:
        nclass = max(y) + 1
        dummy = np.array([1 * (y == m) for m in range(nclass) ]).transpose()
